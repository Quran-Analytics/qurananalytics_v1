...
```{r ch7fig706, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Plot of the degree distribution for Saheeh"}
...
ggplot(data.frame("x" = 1:length(top_degree),"y"=top_degree )) + 
  geom_point(aes(x=x,y=y),color = 'red', show.legend = F) + 
  scale_y_log10() +
...
```{r ch7fig716, fig.align="center",fig.height=5,fig.width=7, echo=T,warning=F, fig.cap="Keyword in context (kwic) plot for Surah Maryam and Al-Kahf for the word 'Allah' and 'Lord'"}
...
ggplot() + 
  ...
  geom_point(aes(x = (1:nrow(tstjac_maryam)), y = tstjac_maryam$jaccard), color = "#3B9D47") +
  labs(x = "Edges", y = "Similarity Score",
       title = "Verses Similarity for Surah Maryam",
       subtitle = "Using <span style = 'color: red;'>Correlation</span>, <span style = 'color: blue;'>Cosine</span> and <span style = 'color: #31783A;'>Jaccard</span> measures")+
  theme(plot.subtitle = element_markdown())
```
...
ggraph(igph_maryamcos, layout = "kk") +
  ...
  geom_node_text(aes(label = name), col = "black", size = 4)+
  theme_bw()+
  labs(title = "Verses with High Similarity for Surah Maryam")
```
...
ggraph(igph_maryamcos, layout = "kk") +
  geom_edge_link(aes(width = manhattan, edge_alpha = manhattan), edge_color = "#54B660") +
  geom_node_point(size = 0.1, shape = 1, color = "black") +
  geom_node_text(aes(label = name), col = "black", size = 4) +
  labs(title = "Verses with High Distance for Surah Al-Kahf")
```
