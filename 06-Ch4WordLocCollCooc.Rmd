...
ESI_bigrams_filtered %>%
   ...
   geom_col(fill = "#79ad95",color = 'black') +
   coord_flip() +
   theme_bw()+
   theme(axis.text = element_text( 
     ...
     color="blue", 
     size=10)) +
   labs(x = NULL, y = "Counts",title = "Top Words in Relation with 'Allah' in the Quran")
```
\normalsize
...
```{r ch4fig402, fig.align="center",fig.height=6,fig.width=8, echo=F,warning=F, fig.cap="Top word pairs tf-idf in the long Surahs"}
...
stats %>% ggplot() + 
  geom_bar(aes(x = key, y = freq), stat = "identity", fill = "#79ad95",color = 'black') + 
  theme_bw() + 
  coord_flip() +
  labs(y = "Frequency",
       x = "Keywords")
...
stats %>% head(20) %>% ggplot() + 
  geom_bar(aes(x = key, y = freq), 
           stat = "identity", fill = "#79ad95",color = 'black') + 
  theme_bw() + 
  coord_flip() +
  labs(y = "Frequency",
       x = "Keywords")
