...
```{r ch3fig301, fig.align="center",fig.height=5,fig.width=6, echo=T,warning=F, fig.cap="Top common words in the translations"}
...
```{r ch3fig302, fig.align="center",fig.height=4,fig.width=8, echo=T,warning=F, fig.cap="Bing's top negative and positive words"}
...
ch3_afinn_plot = function(sent_df,title_label){
  sent_df %>%
      ...
      labs(y = "n", x = NULL,
           title = paste0("AFINN Scoring Model on ",title_label)) +
      coord_flip() +
      theme(axis.text = element_text( 
                          angle = 0, 
                          color="blue", 
...
ch3_bing_plot3 = function(sent_df,title_label){
   sent_df %>%
      ...
      top_n(20) %>%
      ggplot(aes(x = reorder(surah_title_en,ratio), y = ratio)) +
      geom_col(fill = '#2B9D5A',color = 'black') +
...
all_surahs_sent %>%       
    ggplot() + 
    geom_col(aes(x = ayah_id, y = score, fill = sentiment,width = 3 ),
             show.legend = FALSE ) +
  geom_hline(yintercept = 0)+
  coord_cartesian(ylim = c(-15,10))+
    labs(x = NULL, y = "Sentiment Score", title = "Sentiment Frequency in Saheeh Corpus") + 
...
sel_surahs_sent = tidyESIC %>% 
    ...
    inner_join(get_sentiments("bing")) %>% 
    mutate(score = if_else(sentiment=="positive",1,-1)) %>% 
    mutate(sentiment = if_else(score < 0, "firebrick",'#2B9D5A')) 
...
cowplot::plot_grid(p1,p2,p3,p4,p5,p6, nrow = 3, ncol = 2)
```
