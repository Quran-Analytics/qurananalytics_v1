...
% title section
	...
	{\LARGE QURAN ANALYTICS WITH R\\} % Title
	\vspace{0.75\baselineskip} 
	{\LARGE A Practical Introduction\\} % Subtitle
   ...
   Authors \\
	...
	{\scshape\Large Wan M Hasni \\ Azman Hussin \\} % Authors list
	\vspace{0.5\baselineskip} % Whitespace below the author list
	\textit{LeadUS \\ Malaysia} % Author affiliation
	\vfill % Whitespace between author names and publisher logo
	%	Publisher
	\vspace{0.3\baselineskip} % Whitespace under the publisher logo
	2021 % Publication year
	{\large unpublished} % Publisher
...
library(knitr)
knitr::opts_chunk$set(message = FALSE, warning = FALSE, error = FALSE, size = "small")
...
\chapter*{Preface From the First Author}\label{chap:preface1}
\begin{center}
\textarabic{نٓۚ وَٱلۡقَلَمِ وَمَا يَسۡطُرُونَ}

\textit{"Nun. By the Pen (Al-Qalam) and what they inscribe" [Al-Qalam:1]}
\end{center}
...
\chapter*{Preface From the Second Author}\label{chap:preface2}
\begin{center}
\textarabic{خَيْرُكُمْ مَنْ تَعَلَّمَ الْقُرْآنَ وَعَلَّمَهَُا يَسۡطُرُونَ}

\textit{"The best of you are those who learn the Quran and teach it."}
\end{center}
...
\begin{center}
\textarabic{وَرَبُّنَا الرَّحْمَٰنُ الْمُسْتَعَانُ عَلَىٰ مَا تَصِفُونََصِفُونَ}

\textit{"And our Lord is the Beneficent Allah, Whose help is sought" [Quran, 21:112]}
\end{center}

```{r fig.align="center",fig.height=3,fig.width=4, echo=F,warning=F}
grid.raster(readJPEG("images/AzmanSSM02.jpg"))
```
...
\mainmatter
\chapter{Introducing Quran Analytics}\label{chap:chap1}
...
Quran Analytics is a new term without a clear scholarly definition. We know that it is a combination of various disciplines - including Quranic sciences (Ulum Al-Quran), language and linguistics, and broader Islamic sciences. The relationship and interdependencies between these fields are both _complicated_ and _complex_. Our exploratory attempt to provide a diagrammatic relation between the various fields of knowledge is shown in Figure 1.1.

```{r, echo=F,fig.cap="Complexity of Quran Analytics"}
img_qa = readPNG("images/QA_ch1_1A.png")
grid.raster(img_qa)
```
...
## Quranic language and linguistic studies
...
## Advancements in NLP and Quran Analytics 
...
\part{Analysis of Words by its Frequencies}\label{part:part1}
...
This part consists of two chapters, Chapter \@ref(chap:chap2) and Chapter \@ref(chap:chap3). In Chapter \@ref(chap:chap2) we will cover the statistical analysis of translations of Al-Quran. In Chapter \@ref(chap:chap3) we choose one of the methods to deal with words by applying selected word scoring models, namely sentiment scoring. A full exercise for both tasks requires lengthy expositions, which is not our intention. Instead, we provide a sketch of ideas of possible works as a guide for full-scale analysis.
...
\chapter{Word Frequency Analysis}\label{chap:chap2}
...
hamim_surahs = c("Ghafir", "Fussilat", "Ash-Shura", "Az-Zukhruf", 
                 "Ad-Dukhaan", "Al-Jaathiya")
long_surahs = c("Al-Baqara", "Aal-i-Imraan", "An-Nisaa", "Al-Maaida", 
                "Al-An'aam", "Al-A'raaf")
```
\normalsize
...
## Words of high occurrence and stopwords
...
\chapter{Word Scoring Analysis}\label{chap:chap3}
...
Since we will use the same dataset that as in Chapter \@ref(chap:chap2), and the _tidytext_ package, readers can assume that the computing environment in Chapter \@ref(chap:chap2) is continued here. 

## Pre-processing the data

All the works in __R__ for this chapter are just a continuation of the previous computing environment. Before we proceed, we have to decide whether to remove stopwords from the texts or to retain them. The reasons for removing stopwords are explained in Chapter \@ref(chap:chap2), to remove frequently occurring words that carry no further meaning, except for continuation of the sentence. In _tidytext_ the English stopwords are pre-prepared from three lexicons as _stop_words_ variable. We can use them all together, as we have here, or use the _filter()_ functions to only use one set of stop words if that is more appropriate for certain analysis.
...
|---------------|----------------------|--------------------|----------------------|---------------------|
| Saheeh   | `r comma(nrow(tidyESI))`|`r comma(nrow(tidyESIC))`| `r comma(length(unique(tidyESI$word)))` | `r comma(length(unique(tidyESIC$word)))` |
| Yusuf Ali | `r comma(nrow(tidyEYA))`|`r comma(nrow(tidyEYAC))`| `r comma(length(unique(tidyEYA$word)))` | `r comma(length(unique(tidyEYAC$word)))` |

We can see that the words consist of `r 100 - round(100*nrow(tidyESIC)/nrow(tidyESI),2)` percent of stopwords in Saheeh, and `r 100 - round(100*nrow(tidyEYAC)/nrow(tidyEYA),2)` percent in Yusuf Ali. That is a significant number of words. On the other hand the stopwords tokens removed are only `r 100 - round(100*length(unique(tidyESIC$word))/length(unique(tidyESI$word)),2)` percent for Saheeh, and `r 100 - round(100*length(unique(tidyEYAC$word))/length(unique(tidyEYA$word)),2)` percent for Yusuf Ali, which is much lesser in percentage terms.
...
\footnotesize
```{r ch3fig301, fig.align="center",fig.height=4,fig.width=4, echo=T,warning=F, fig.cap="Top common words in the translations"}
...
In Chapter \@ref(chap:chap2), we explored the tidy text format and showed it can easily be used to approach questions about word frequency in the English Quran. This allowed us to analyze which words are used most frequently in the Quran and to compare two versions of the English Quran.
...
\
\footnotesize
```{r ch3fig302, fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Bing's top negative and positive words"}
...
\
\footnotesize
```{r ch3fig303, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="AFINN scoring model on Saheeh"}
afinn_word_counts_ESIC <- tidyESIC %>%
  inner_join(get_sentiments("afinn")) %>%
  count(word, value, sort = TRUE) %>%
  ungroup()
afinn_word_counts_EYAC <- tidyEYAC %>%
  inner_join(get_sentiments("afinn")) %>%
  count(word, value, sort = TRUE) %>%
  ungroup()

ch3_afinn_plot = function(sent_df,title_label){
  sent_df %>%
      group_by(value) %>%
      top_n(7) %>% ungroup() %>%
      mutate(word = reorder(word, n)) %>%
...
ch3_afinn_plot(afinn_word_counts_ESIC,"Saheeh")
```

```{r ch3fig304, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="AFINN scoring model on Yusuf Ali"}
ch3_afinn_plot(afinn_word_counts_EYAC,"Yusuf Ali")
```
\normalsize
...
## Sentiment analysis within the Surahs
...
\
\footnotesize
```{r ch3fig305, fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Negative Surahs scoring"}
...
\
\footnotesize
```{r ch3fig306,fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Positive Surahs scoring"}
...
ch3_bing_plot3 = function(sent_df,title_label){
   sent_df %>%
...
p1 = ch3_bing_plot3(tidyESIC,"Saheeh")
p2 = ch3_bing_plot3(tidyEYAC,"Yusuf Ali")
...
\
\footnotesize
```{r ch3fig307,fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Sentiment scoring within Surah Yusuf"}
...
\
\footnotesize
```{r ch3fig308, fig.align="center",fig.height=3,fig.width=4, echo=T,warning=F, fig.cap="Comparison cloud for Saheeh translation"}
...
```{r ch3fig309,fig.align="center",fig.height=3,fig.width=4, echo=F,warning=F, fig.cap="Comparison cloud for Yusuf Ali translation"}
tidyEYAC %>%
  inner_join(get_sentiments("bing")) %>%
  count(word, sentiment, sort = TRUE) %>%
  acast(word ~ sentiment, value.var = "n", fill = 0) %>%
...
|                |  Saheeh                   | Yusuf Ali |
|----------------|---------------------------|-----------|
| Positive words | `r comma(sent_ratio_all$si_pos[2])`  | `r comma(sent_ratio_all$ya_pos[2])` |
| Negative words | `r comma(sent_ratio_all$si_pos[1])`  | `r comma(sent_ratio_all$ya_pos[1])` |
| Percent positive | `r round(sent_ratio_all$si_pos[2]/(sent_ratio_all$si_pos[2]+sent_ratio_all$si_pos[1]),2)` | `r round(sent_ratio_all$ya_pos[2]/(sent_ratio_all$ya_pos[2]+sent_ratio_all$ya_pos[1]),2)` |
| Percent negative |  `r round(sent_ratio_all$si_pos[1]/(sent_ratio_all$si_pos[2]+sent_ratio_all$si_pos[1]),2)` | `r round(sent_ratio_all$ya_pos[1]/(sent_ratio_all$ya_pos[2]+sent_ratio_all$ya_pos[1]),2)` |

A first observation is: "warnings" (negative sentiment) exceeds "glad tidings" (positive sentiment) by about `r 100*(0.57-0.43)` percent consistently in both translations. If we use the ratio of appearance of the word "Basheeran" versus "Nazeeran", which is `r round(18/58,2)` percent, as an indicator of emphasis, then the finding is consistent. This is to say that Al-Quran emphasizes more on the "warnings" than the "glad tidings". A simple casual first observation is that the words used in the two English translations of the Quran are "biased" towards negative sentiments. (Obviously, our readers will say that this casual finding must be verified with the original Quran in Arabic.)
...
|                       |  Saheeh                   | Yusuf Ali |
|-----------------------|---------------------------|-----------|
| Positive words Meccan | `r comma(sent_ratio_rev$si_pos_meccan[2])`  | `r comma(sent_ratio_rev$ya_pos_meccan[2])` |
| Negative words Meccan | `r comma(sent_ratio_rev$si_pos_meccan[1])`  | `r comma(sent_ratio_rev$ya_pos_meccan[1])` |
| Percent positive Meccan | `r round(sent_ratio_rev$si_pos_meccan[2]/(sent_ratio_rev$si_pos_meccan[2]+sent_ratio_rev$si_pos_meccan[1]),2)` | `r round(sent_ratio_rev$ya_pos_meccan[2]/(sent_ratio_rev$ya_pos_meccan[2]+sent_ratio_rev$ya_pos_meccan[1]),2)` |
| Percent negative Meccan | `r round(sent_ratio_rev$si_pos_meccan[1]/(sent_ratio_rev$si_pos_meccan[2]+sent_ratio_rev$si_pos_meccan[1]),2)` | `r round(sent_ratio_rev$ya_pos_meccan[1]/(sent_ratio_rev$ya_pos_meccan[2]+sent_ratio_rev$ya_pos_meccan[1]),2)` |
|---------------|---------------------------|------------|
| Positive words Medinan | `r comma(sent_ratio_rev$si_pos_medinan[2])`  | `r comma(sent_ratio_rev$ya_pos_medinan[2])` |
| Negative words Medinan | `r comma(sent_ratio_rev$si_pos_medinan[1])`  | `r comma(sent_ratio_rev$ya_pos_medinan[1])` |
| Percent positive Medinan | `r round(sent_ratio_rev$si_pos_medinan[2]/(sent_ratio_rev$si_pos_medinan[2]+sent_ratio_rev$si_pos_medinan[1]),2)` | `r round(sent_ratio_rev$ya_pos_medinan[2]/(sent_ratio_rev$ya_pos_medinan[2]+sent_ratio_rev$ya_pos_medinan[1]),2)` |
| Percent negative Medinan | `r round(sent_ratio_rev$si_pos_medinan[1]/(sent_ratio_rev$si_pos_medinan[2]+sent_ratio_rev$si_pos_medinan[1]),2)` | `r round(sent_ratio_rev$ya_pos_medinan[1]/(sent_ratio_rev$ya_pos_medinan[2]+sent_ratio_rev$ya_pos_medinan[1]),2)` |
...
\
\footnotesize
```{r ch3fig310,fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Sentiment frequency in Saheeh corpus"}
all_surahs_sent = tidyESIC %>% 
    inner_join(get_sentiments("bing")) %>% 
    mutate(score = if_else(sentiment=="positive",1,-1)) %>% 
    mutate(sentiment = if_else(score > 0, "red","green")) 

all_surahs_sent %>%       
    ggplot() + 
    geom_col(aes(x = ayah_id, y = score, fill = sentiment ),
             show.legend = FALSE ) +
...
\
\footnotesize
```{r ch3fig311,fig.align="center",fig.height=8,fig.width=6, echo=F,warning=F, fig.cap="Sentiment frequency in selected Surahs"}
sel_surahs = c("Yaseen","Al-Fath","Al-Mulk","Al-Muddaththir","Al-Waaqia","Al-Qiyaama")
sel_surahs_sent = tidyESIC %>% 
    filter(surah_title_en %in% sel_surahs) %>% 
    inner_join(get_sentiments("bing")) %>% 
    mutate(score = if_else(sentiment=="positive",1,-1)) %>% 
    mutate(sentiment = if_else(score > 0, "red","green")) 

sent_plotter = function(sent_df, plot_surah) {
  sent_df %>% filter(surah_title_en == plot_surah) %>% 
      ggplot() + geom_col(aes(x = ayah_id, y = score, fill = sentiment ), show.legend = FALSE ) +
      labs(x = NULL, y = NULL) + theme(legend.position = "none") +
      facet_wrap(~surah_title_en) 
}

p1 = sent_plotter(sel_surahs_sent, sel_surahs[1])
p2 = sent_plotter(sel_surahs_sent, sel_surahs[2])
p3 = sent_plotter(sel_surahs_sent, sel_surahs[3])
p4 = sent_plotter(sel_surahs_sent, sel_surahs[4])
p5 = sent_plotter(sel_surahs_sent, sel_surahs[5])
p6 = sent_plotter(sel_surahs_sent, sel_surahs[6])
cowplot::plot_grid(p1,p2,p3,p4,p5,p6, nrow = 6, ncol = 2)
```
\normalsize
...
## Building dedicated sentiment scoring model
...
\part{Analysis of Words by its Cooccurences}\label{part:part2}
...
As an introduction to the subject, in the next three chapters, we will address the subject of the relationship between words from three different perspectives. The first perspective is from the locations of words with other words in the corpus, or "collocations" and "co-occurrences" (Chapter \@ref(chap:chap4)). The second perspective is to look at these relations from statistical aspects, namely the correlations and other statistical measures (Chapter \@ref(chap:chap5)). The third perspective is to look at the relationship from a holistic or global approach, namely by looking at all of the words as a network of words (Chapter \@ref(chap:chap6)).
...
\chapter{Word Collocations}\label{chap:chap4}
...
## Finding keyword combinations using POS
...
\footnotesize
``` {r}
y <- c("PROPN", "SCONJ", "ADJ", "NOUN", "VERB", "INTJ", "DET",  
       "AUX", "NUM", "X", "PRON", "PUNCT", "ADP", "CCONJ")
as_phrasemachine(y)
```
\normalsize
...
QSI_udp = readRDS("data/QSI_udp_s.RDS") 
QYA_udp = readRDS("data/QYA_udp.RDS")
```
\normalsize

## Statistical analysis of word positions
...
\
```{r ch5fig501, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Yusuf Ali for the entire text"}
all_surah = as.character(1:114)
p1 = pos_plotter(QSI_udp,all_surah,"Saheeh")  + theme(legend.position = "none")
p2 = pos_plotter(QYA_udp,all_surah,"Yusuf Ali")  + theme(legend.position = "none")
cowplot::plot_grid(p1,p2)
```
...
\
\footnotesize
```{r ch5fig502, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Yusuf Ali for early Surahs"}
p1 = pos_plotter(QSI_udp,early_surah_no,"Saheeh")
p2 = pos_plotter(QYA_udp,early_surah_no,"Yusuf Ali")
cowplot::plot_grid(p1,p2)
```
```{r ch5fig503, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Yusuf Ali for middle Surahs"}
p1 = pos_plotter(QSI_udp,mid_surah_no,"Saheeh")
p2 = pos_plotter(QYA_udp,mid_surah_no,"Yusuf Ali")
cowplot::plot_grid(p1,p2)
```

```{r ch5fig504, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS in Saheeh and Yusuf Ali for last Surahs"}
p1 = pos_plotter(QSI_udp,last_surah_no,"Saheeh")
p2 = pos_plotter(QYA_udp,last_surah_no,"Yusuf Ali")
cowplot::plot_grid(p1,p2)
```
\normalsize
...
```{r, echo=FALSE}
QAR_udp = readRDS("data/QAR_udp.RDS")
```
...
|-----------------|---------|---------|-----------|
| PUNCTUATION     |`r comma(nrow(QAR_udp[QAR_udp$upos == "PUNCT",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "PUNCT",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "PUNCT",]))` |
| NOUN            |`r comma(nrow(QAR_udp[QAR_udp$upos == "NOUN",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "NOUN",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "NOUN",]))` |
| PRONOUN         |`r comma(nrow(QAR_udp[QAR_udp$upos == "PRON",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "PRON",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "PRON",]))` |
| VERB            |`r comma(nrow(QAR_udp[QAR_udp$upos == "VERB",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "VERB",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "VERB",]))` |
| X               |`r comma(nrow(QAR_udp[QAR_udp$upos == "X",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "X",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "X",]))` |
| ADPOSITION      |`r comma(nrow(QAR_udp[QAR_udp$upos == "ADP",]))` | `r comma(nrow(QSI_udp[QSI_udp$upos == "ADP",]))` | `r comma(nrow(QYA_udp[QYA_udp$upos == "ADP",]))` |
|-----------------|---------|---------|------------|
| Total  tokens   | `r comma(nrow((QAR_udp[!is.na(QAR_udp$feats),])))`  | `r comma(nrow((QSI_udp[!is.na(QSI_udp$feats),])))` | `r comma(nrow((QYA_udp[!is.na(QYA_udp$feats),])))` |
| Unique tokens   | `r comma(length(unique(QAR_udp$token)))`     | `r comma(length(unique(QSI_udp$token)))` | `r comma(length(unique(QYA_udp$token)))` |
...
```{r ch5fig505, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Arabic for the entire text"}
p1 = pos_plotter(QSI_udp,all_surah,"Saheeh")  + theme(legend.position = "none")
p2 = pos_plotter(QAR_udp,all_surah,"Arabic")  + theme(legend.position = "none")
cowplot::plot_grid(p1,p2)
```
...
```{r ch5fig506, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Arabic for the early Surahs"}
p1 = pos_plotter(QSI_udp,early_surah_no, "Saheeh")
p2 = pos_plotter(QAR_udp,early_surah_no, "Arabic")
cowplot::plot_grid(p1,p2)
```
```{r ch5fig507, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Arabic for the middle Surahs"}
p1 = pos_plotter(QSI_udp,mid_surah_no, "Saheeh")
p2 = pos_plotter(QAR_udp,mid_surah_no, "Arabic")
cowplot::plot_grid(p1,p2)
```

```{r ch5fig508, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Saheeh and Arabic for the last Surahs"}
p1 = pos_plotter(QSI_udp,last_surah_no,"Saheeh")
p2 = pos_plotter(QAR_udp,last_surah_no,"Arabic")
cowplot::plot_grid(p1,p2)
```
...
```{r ch5fig509, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Percentage of POS categories in Surah Yusuf"}

a = QSI_udp %>% filter(surah_no %in% "12") %>% 
                group_by(surah_no) %>% count(upos) %>% 
                mutate(prop=n/sum(n)) 
b = QYA_udp %>% filter(surah_no %in% "12") %>% 
                group_by(surah_no) %>% count(upos) %>% 
                mutate(prop=n/sum(n)) 
a$version = "saheeh"; b$version = "yusufali"; 
abc_df = bind_rows(a,b)

abc_df %>%      
ggplot(aes(x = as.factor(upos), y=prop, color = version)) +
                  geom_point() + scale_color_brewer(palette = "Dark2") +
                  theme(axis.text.x = element_text(angle = 90), 
                        legend.position = "top") +
                  ylim(0,0.3) +
                  labs(x = "UPOS", y = "percentage")
```
...
```{r, echo=FALSE}
## This is not show, and spare as reserve
sy_ar = QAR_udp %>% filter(surah_no %in% "12")
QAR_cooc <- cooccurrence(x = subset(sy_ar, upos %in% c("NOUN", "PROPN", "VERB")), 
                     term = "lemma", 
                     group = c("ayah_no", "paragraph_id", "sentence_id"))
QAR_cooc_g <- graph_from_data_frame(head(QAR_cooc,100))
```
\normalsize
...
```{r ch5fig510, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Co-occurrence network for top relations in Surah Yusuf for Saheeh"}
set.seed(1234)
gg_plotter1(QSI_cooc_g)
```

```{r ch5fig511, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Co-occurrence network for top relations in Surah Yusuf for Yusuf Ali"}
set.seed(2345)
gg_plotter1(QYA_cooc_g)
```
...
\
\footnotesize
```{r ch5fig512, fig.align="center",fig.height=3,fig.width=4, echo=T,warning=F, fig.cap="Arc view of co-occurrence network for top relations in Surah Yusuf for Saheeh"}
...
```{r ch5fig513, fig.align="center",fig.height=3,fig.width=4, echo=F,warning=F, fig.cap="Arc view of co-occurrence network for top relations in Surah Yusuf for Yusuf Ali"}
ggraph(QYA_cooc_g, layout = 'linear') + 
    geom_edge_arc(color = "gold3", width=0.5) +
    geom_node_point(size=2, color="black") +
    geom_node_text(aes(label = name), repel=TRUE, size = 4) +
    theme_void()
...
```{r ch5fig514, fig.align="center",fig.height=3,fig.width=4, echo=F,warning=F, fig.cap="Arc view of co-occurrence network for top relations in Surah Yusuf for Arabic"}
ggraph(QAR_cooc_g, layout = 'linear') + 
    geom_edge_arc(color = "gold3", width=0.5) +
    geom_node_point(size=2, color="black") +
...
\
\footnotesize
```{r ch5fig515, fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Circular view of co-occurrence network for top relations in Surah Yusuf for Saheeh"}
ggraph(QSI_cooc_g, layout = 'linear', circular = TRUE) + 
     ...
     theme_void()
```
```{r ch5fig516, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Circular view of co-occurrence network for top relations in Surah Yusuf for Yusuf Ali"}
ggraph(QYA_cooc_g, layout = 'linear', circular = TRUE) + 
     geom_edge_arc(color = "gold3", width=0.5) +
     geom_node_point(aes(size = degree(QYA_cooc_g)), 
                     alpha = igraph::degree(QYA_cooc_g), 
                     colour = "#a83268") +
     geom_node_text(aes(label = name), size = 3, repel=TRUE) +
     theme_void()
```
...
\footnotesize
```{r ch5fig517, fig.align="center",fig.height=4,fig.width=6, echo=F,warning=F, fig.cap="Circular view of co-occurrence network for top relations in Surah Yusuf for Arabic texts"}
ggraph(QAR_cooc_g, layout = 'linear', circular = TRUE) + 
     geom_edge_arc(color = "gold3", width=0.5) +
     geom_node_point(aes(size = degree(QAR_cooc_g)), 
                     alpha = igraph::degree(QAR_cooc_g), 
                     colour = "#a83268") +
     geom_node_text(aes(label = name), size = 3, repel=TRUE) +
     theme_void()
```
...
\
\footnotesize
```{r ch5fig518, fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Groupings by fastgreedy in Surah Yusuf for Saheeh"}
...
\
\
```{r ch5fig519, fig.align="center",fig.height=4,fig.width=6, echo=T,warning=F, fig.cap="Groupings by fastgreedy in Surah Yusuf for Yusuf Ali"}
...
## A short tutorial on graphs in __R__
...
\
\footnotesize
```{r ch5tut1, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Co-occurence within sentence"}
library(igraph)
library(ggraph)
library(ggplot2)
wordnetwork <- head(cooccur, 100)
...
\
\footnotesize
``` {r ch5tut2, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="First plot of tutorial word network"}
plot(gm)
```
\normalsize
...
\
\footnotesize
``` {r ch5tut3, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust some edge parameters"}
...
\
\footnotesize
``` {r ch5tut4, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust colors"}
...
\
\footnotesize
``` {r ch5tut5, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust node size based on its degree"}
...
\
``` {r ch5tut6, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Remove node labels"}
deg <- degree(gm, mode="all")
V(gm)$size <- deg
...
\
\footnotesize
``` {r ch5tut7, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust edge width based on number of co-occurrences"}
E(gm)$cooc[1:10]
...
\
\footnotesize
``` {r ch5tut8, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust edge color and set layout"}
...
\
\footnotesize
``` {r ch5tut9, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust nodes highlighting only labels"}
...
\
\footnotesize
``` {r ch5tut10, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust edge color based on the source node"}
...
\
\footnotesize
``` {r ch5tut11, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust edge color based on formula"}
...
\
\footnotesize
``` {r ch5tut12, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Adjust layout and remove label"}
...
\
\footnotesize
``` {r ch5tut13, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using dh layout"}
l<-layout_with_dh(gm)
...
\
\footnotesize
``` {r ch5tut14, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Layout in circle"}
...
\
\footnotesize
``` {r ch5tut15, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Layout as tree"}
plot(gm, vertex.color=vertex_attr(gm)$cor,
     vertex.size=igraph::degree(gm),
     edge.width=(edge_attr(gm)$weight)/100, 
     edge.color="grey50", 
...
\
\footnotesize
``` {r ch5tut16, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Layout as star"}
plot(gm, vertex.color=vertex_attr(gm)$cor,
     vertex.size=igraph::degree(gm),
...
\
\footnotesize
``` {r ch5tut17, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F,results = "hide", fig.cap="Additional layouts"}
...
Various plot outputs are in Figure \@ref(fig:ch5tut17).
...
\
\footnotesize
``` {r ch5tut18, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using fr layout"}
l <- layout_with_fr(gm)
plot(gm, layout=l)
```
\normalsize
...
\
\footnotesize
``` {r ch5tut19, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using fr layout with 50 iterations"}
...
\
\footnotesize
``` {r ch5tut20, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using fr layout different weights"}
...
\
\footnotesize
``` {r ch5tut21, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Each fr layout call with different outcomes"}
...
\
\footnotesize
``` {r ch5tut22, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using fr layout call with manual rescaling"} 
l <- layout_with_fr(gm)
...
\
\footnotesize
``` {r ch5tut23, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using kk layout"}
l <- layout_with_kk(gm)
plot(gm, layout=l)
```
\normalsize
...
\
\footnotesize
``` {r ch5tut24, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using graphopt layout"}
...
\
\footnotesize
``` {r ch5tut25, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using graphopt layout with different charge"}
...
\
\footnotesize
``` {r ch5tut26, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using lgl layout with different roots"}
par(mfrow=c(1,2), mar=c(1,1,1,1))
...
\
\footnotesize
``` {r ch5tut27, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using mds layout"}
...
\
\footnotesize
``` {r ch5tut28, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Communities or clusters in tutorial word network"}
...
\
\footnotesize
``` {r ch5tut29, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Path from one node to another"}
...
\
\footnotesize
``` {r ch5tut30, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with minimal parameters set"}
library(ggraph)
...
\
\footnotesize
``` {r ch5tut31, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="ggraph with node and edge parameters set"}
ggraph(lay) + 
    geom_edge_link() + 
    geom_node_point() + 
    geom_node_text(aes(label = name), repel=TRUE) +
...
\
\footnotesize
``` {r ch5tut32, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="ggraph with node size and alpha based on degree"}
ggraph(lay) + 
    geom_node_point() + 
    geom_node_text(aes(label = name), repel=TRUE) +
...
\
\footnotesize
``` {r ch5tut33, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="ggraph with edge width and alpha based on number of co-occurrences"}
ggraph(lay) + 
...
\
\footnotesize
``` {r ch5tut34, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with layout kk"} 
ggraph(gm, layout="kk") +
  geom_edge_link() +
...
\
\footnotesize
``` {r ch5tut35, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with linear layout and void theme"} 
...
\
\footnotesize
``` {r ch5tut36, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with kk layout and aes setting"}
ggraph(gm, layout="kk") +
  geom_edge_link(aes(color = cooc)) +           
...
\
\footnotesize
``` {r ch5tut37, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with various parameter settings"}
ggraph(gm,  layout = 'kk') +
...
\
\footnotesize
``` {r ch5tut38, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with linear layout"}
...
\
\footnotesize
``` {r ch5tut39, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with edge fan"}
ggraph(gm, layout = 'kk') + 
...
\
\footnotesize
``` {r ch5tut40, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Using ggraph with edge density"}
ggraph(gm, layout = 'kk') + 
...
\
\footnotesize
``` {r ch5tut41, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Heatmap of word network"}
...
\
\footnotesize
``` {r ch5tut42, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Degree distribution of word network"}
...
\
\footnotesize
``` {r ch5tut43, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Larger network density with labels"}
wordnetwork <- head(cooccur, 100)
...
\
``` {r ch5tut44, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Larger network density without labels"}
ggraph(gm1, layout = 'kk') + 
      geom_edge_density(aes(fill = cooc)) + 
      geom_edge_link(alpha = 0.7, color = "#57d3e6") +
...
\
\footnotesize
``` {r ch5tut45, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Digital art from Surah Yusuf"}
ggraph(gm1, layout = 'kk') + 
      geom_edge_density(aes(fill = cooc)) + 
      geom_edge_link(alpha = 0.7, color = "#57d3e6") +
      geom_node_point(aes(size = degree(gm1)),
...
\
\footnotesize
``` {r ch5tut46, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Word network with circular layout"}
...
\
\footnotesize
``` {r ch5tut47, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Larger word network with kk layout and edge density"}
...
``` {r ch5tut48, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Larger word network with kk layout and edge-in density"}
...
\
\footnotesize
``` {r ch5tut49, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Larger word network with kk layout and edge-out density"}
...
\
\footnotesize
``` {r ch5tut50, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="One big cluster and unconnected nodes"}
...
\
\footnotesize
``` {r ch5tut51, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Communities within the large fully connected network"}
...
\
\footnotesize
``` {r ch5tut52, fig.align="center",fig.height=4,fig.width=6, echo=T, warning=F, fig.cap="Another piece of art from Surah Yusuf"}
...
\
\footnotesize
``` {r ch6fig601, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="First plot of tutorial word network"}
wordnetwork <- head(cooccur, 50)
wordnetwork <- graph_from_data_frame(wordnetwork)
...
\
\footnotesize
``` {r ch6fig602, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Second plot of tutorial word network"}
ggraph(gd, layout = "kk") +
...
\
\footnotesize
```{r ch6fig603, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap = "Centrality summary"}
img_cs = readJPEG("images/centrality_summary.jpg")
grid.raster(img_cs)
```
\normalsize
...
\
\footnotesize
``` {r ch6fig604, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Node size reflects degree"}
degree(gd)[1:5]
degree(gd)[6:10]
set.seed(10)
...
\
\footnotesize
``` {r ch6fig605, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Node size reflects graph.strength"}
set.seed(10)
...
\
\footnotesize
``` {r ch6fig606, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Degree distribution of tutorial word network"}
...
\
\footnotesize
``` {r ch6fig607, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="In degree distribution of directed word network"}
...
``` {r ch6fig608, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Out degree distribution of directed word network"}
...
\
\footnotesize
``` {r ch6fig609, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Node size reflects betweenness centrality"}
...
\
\footnotesize
``` {r ch6fig610, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Simple Scatterplot Matrix"}
...
\
\footnotesize
``` {r ch6fig611, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Relationship degree and strength"}
#assemble dataset
names = V(gd)$name
deg = degree(gd)
st = graph.strength(gd)
...
\
\footnotesize
``` {r ch6fig612, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Relationship betweenness and strength"}
       
dfd %>% ggplot(aes(x = strength, y = betweenness)) + 
...
For *gd* the number of nodes _n_ is `r vcount(gd)` and the number of edges _m_ is `r ecount(gd)`.
For *gu* the number of nodes _n_ is `r vcount(gu)` and the number of edges _m_ is `r ecount(gu)`.
...
\
\footnotesize
``` {r ch6fig613, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F,  fig.cap="Simple plot showing network components"}
...
\footnotesize
``` {r ch6fig614, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F,  fig.cap="Histogram of gd degrees"}
hist(degree(gd), breaks=5, col="steelblue")
```
``` {r ch6fig615, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F,  fig.cap="Histogram of gu degrees"}
...
\footnotesize
``` {r ch6fig616, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F,  fig.cap="Degree distribution of gd"}
pkd = degree.distribution(gd)
plot(pkd, pch=20)
```
``` {r ch6fig617, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F,  fig.cap="Degree distribution of gu"}
...
\footnotesize
``` {r ch6fig618, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Layout plot of gu"}
set.seed(7)
l = layout_with_kk(gu)
plot(gu, layout=l, edge.color="cyan4")
```
\normalsize
...
\footnotesize
``` {r ch6fig619, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Community structure using edge.betweenness.community()"}
plot(eb, gu, layout=l)
```
\normalsize
...
\
\footnotesize
``` {r ch6fig620, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Community structure using the Louvain method"}
...
\
\footnotesize
``` {r ch6fig621, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Using the Louvain method with some color adjustments"}
library(RColorBrewer)
...
\
\footnotesize
``` {r ch6fig622, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Node size reflecting its degree"}
set.seed(3)
l = layout_with_kk(gu)
...
\
\footnotesize
``` {r ch6fig623, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Node size discrete"}
...
\
\footnotesize
``` {r ch6fig624, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Using different node shape"}
set.seed(3)
# create a node trait that varies randomly for all nodes 
V(gu)$random = rnorm(10, mean=20, sd=5) 
...
\
\footnotesize
``` {r ch6fig625, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Simple first plot example using tidygraph and ggraph"}
...
\
\footnotesize
``` {r ch6fig626, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Adjusting node and edge plotting parameters"}
...
\
\footnotesize
``` {r ch6fig627, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Degree for top 30 words"}
       
node_measures %>% arrange(desc(degree)) %>% 
# First sort by degree. This sorts the data.frame but NOT the factor levels
     head(30) %>% 
...
``` {r ch6fig628, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Degree-in and degree-out for top 50 words"}
       
NMtall <- node_measures %>% 
              gather(key = center, value = Value, 
...
``` {r ch6fig629, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Degree and betweenness for top 50 words"}
...
\footnotesize
``` {r ch6fig630, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Other measures for top 50 words"}
...
\footnotesize
``` {r ch6fig631, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Nodes are colored by group"}
gtexample %>%
  activate(nodes) %>% 
  as.tibble()
...
\footnotesize
``` {r ch6fig632, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Nodes are colored by centeredness"}
got_palette = c("red", "blue", "green", "gold", "coral",
                "cyan4", "maroon", "deeppink")
ggraph(gtexample, layout="fr") + 
    geom_edge_density(aes(fill = cooc)) +
    geom_edge_link(aes(width = cooc), alpha = 0.2) + 
...
\
\footnotesize
``` {r ch6fig633, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Top 10 word communities"}
network_ego1 %>%
...
\
\footnotesize
``` {r ch6fig634, fig.align="center", fig.height=4, fig.width=6, echo=T, warning=F, fig.cap="Using draw circle layout with Mose as focus node"}
...
\
\footnotesize
``` {r ch6fig635, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Same layout with say as focus node and text labels"}
deg = degree(jg1)
...
\footnotesize
``` {r ch6fig636, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Weighted degree centrality layout"}
...
\footnotesize
``` {r ch6fig637, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Betweenness centrality layout with degree for node size"}
       
ggraph(jg1,layout = "centrality", 
       cent = betweenness(jg1, directed = F, normalized = T)) +
  geom_edge_link0(aes(edge_width = cooc), edge_color = "hotpink") +
...
\footnotesize
``` {r ch6fig638, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Directed network with clusters"}
cld <- clusters(gd)
...
\footnotesize
``` {r ch6fig639, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Undirected network with clusters"}
...
\footnotesize
``` {r ch6fig640, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Focus layout with only the focus word"}
...
``` {r ch6fig641, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Focus layout with all words"}
...
\footnotesize
``` {r ch6fig642, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Centrality layout : graph.strength with clusters"}
...
``` {r ch6fig643, fig.align="center", fig.height=4, fig.width=6, echo=F, warning=F, fig.cap="Centrality layout : graph.coreness with clusters"}
...
\part{Text and Knowledge Modeling}\label{part:part3}
...
\chapter{Texts Network Analysis}\label{chap:chap7}
...
## Analyzing word co-occurrence as a network
...
\textit{`r paste(names(top_degree[1]),names(top_degree[2]),names(top_degree[3]),names(top_degree[4]),names(top_degree[5]),names(top_degree[6]),names(top_degree[7]), sep= ", ")`}
\textit{`r paste(names(top_degree[8]),names(top_degree[9]),names(top_degree[10]),names(top_degree[11]),names(top_degree[12]),names(top_degree[13]),names(top_degree[14]),sep=", ")`}
\textit{`r paste(names(top_degree[15]),names(top_degree[16]),names(top_degree[17]),names(top_degree[18]),names(top_degree[19]),names(top_degree[20]), sep=", ")`}
...
\textit{`r paste(names(top_btwn[1]),names(top_btwn[2]),names(top_btwn[3]),names(top_btwn[4]),names(top_btwn[5]),sep= ", ")`}
\textit{`r paste(names(top_btwn[6]),names(top_btwn[7]),names(top_btwn[8]),names(top_btwn[9]),names(top_btwn[10]), sep= ", ")`}
\textit{`r paste(names(top_btwn[11]),names(top_btwn[12]),names(top_btwn[13]),names(top_btwn[14]),names(top_btwn[15]), sep = ", ")`}
\textit{`r paste(names(top_btwn[16]),names(top_btwn[17]),names(top_btwn[18]),names(top_btwn[19]),names(top_btwn[20]), sep = " , ")`}
...
\textit{`r paste(names(top_evcent[1]),names(top_evcent[2]),names(top_evcent[3]),names(top_evcent[4]),names(top_evcent[5]), sep = " ,")`}
\textit{`r paste(names(top_evcent[6]),names(top_evcent[7]),names(top_evcent[8]),names(top_evcent[9]),names(top_evcent[10]),sep= " ,")`}
\textit{`r paste(names(top_evcent[11]),names(top_evcent[12]),names(top_evcent[13]),names(top_evcent[14]),names(top_evcent[15]), sep=" ,")`}
\textit{`r paste(names(top_evcent[16]),names(top_evcent[17]),names(top_evcent[18]),names(top_evcent[19]),names(top_evcent[20]),sep=" ,")`}
...
For both Surahs, as shown in Figure \@ref(fig:ch7t50sk) and Figure \@ref(fig:ch7pt50sm), the most prominent word at the center is "lord"; however the surrounding topics are different. In Al-Kahf words such as "cave", "youth", "al-khidh", "moses" emerge, while in Surah Maryam "merciful", "jesus" appear.
...
|  Surah Al-Kahf     |   Text                                                   |
|--------------------|----------------------------------------------------------|
| By degree          |\textit{`r paste(names(top_degreeKahf[1]), names(top_degreeKahf[2]),names(top_degreeKahf[3]),names(top_degreeKahf[4]),names(top_degreeKahf[5]),names(top_degreeKahf[6]),names(top_degreeKahf[7]), sep = " ,")`} |
|                    |\textit{`r paste(names(top_degreeKahf[8]), names(top_degreeKahf[9]),names(top_degreeKahf[10]),names(top_degreeKahf[11]),names(top_degreeKahf[12]),names(top_degreeKahf[13]),names(top_degreeKahf[14]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_degreeKahf[15]), names(top_degreeKahf[16]),names(top_degreeKahf[17]),names(top_degreeKahf[18]),names(top_degreeKahf[19]),names(top_degreeKahf[20]), sep=" ,")`} |
| By betweenness | \textit{`r paste(names(top_btwnKahf[1]), names(top_btwnKahf[2]),names(top_btwnKahf[3]),names(top_btwnKahf[4]),names(top_btwnKahf[5]),names(top_btwnKahf[6]),names(top_btwnKahf[7]), sep = " ,")`} |
|                | \textit{`r paste(names(top_btwnKahf[8]), names(top_btwnKahf[9]),names(top_btwnKahf[10]),names(top_btwnKahf[11]),names(top_btwnKahf[12]),names(top_btwnKahf[13]),names(top_btwnKahf[14]), sep = " ,")`} |
|                | \textit{`r paste(names(top_btwnKahf[15]), names(top_btwnKahf[16]),names(top_btwnKahf[17]),names(top_btwnKahf[18]),names(top_btwnKahf[19]),names(top_btwnKahf[20]), sep = " ,")`}|
| By prestige centrality | "\textit{`r paste(names(top_evcentKahf[1]), names(top_evcentKahf[2]),names(top_evcentKahf[3]),names(top_evcentKahf[4]),names(top_evcentKahf[5]),names(top_evcentKahf[6]),names(top_evcentKahf[7]), sep = " ,")`} |
|                | \textit{`r paste(names(top_evcentKahf[8]), names(top_evcentKahf[9]),names(top_evcentKahf[10]),names(top_evcentKahf[11]),names(top_evcentKahf[12]),names(top_evcentKahf[13]),names(top_evcentKahf[14]), sep = " ,")`} |
|               | \textit{`r paste(names(top_evcentKahf[15]),names(top_evcentKahf[16]),names(top_evcentKahf[17]),names(top_evcentKahf[18]),names(top_evcentKahf[19]),names(top_evcentKahf[20]), sep=" ,")`}|
...
|  Surah Maryam          |   Text                                           |
|--------------------|------------------------------------------------------|
| By degree          | \textit{`r paste(names(top_degreeMaryam[1]),names(top_degreeMaryam[2]),names(top_degreeMaryam[3]),names(top_degreeMaryam[4]),names(top_degreeMaryam[5]),names(top_degreeMaryam[6]),names(top_degreeMaryam[7]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_degreeMaryam[8]),names(top_degreeMaryam[9]),names(top_degreeMaryam[10]),names(top_degreeMaryam[11]),names(top_degreeMaryam[12]),names(top_degreeMaryam[13]),names(top_degreeMaryam[14]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_degreeMaryam[15]),names(top_degreeMaryam[16]),names(top_degreeMaryam[17]),names(top_degreeMaryam[18]),names(top_degreeMaryam[19]),names(top_degreeMaryam[20]), sep=" ,")`}|
| By betweenness     | \textit{`r paste(names(top_btwnMaryam[1]),names(top_btwnMaryam[2]),names(top_btwnMaryam[3]),names(top_btwnMaryam[4]),names(top_btwnMaryam[5]),names(top_btwnMaryam[6]),names(top_btwnMaryam[7]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_btwnMaryam[8]),names(top_btwnMaryam[9]),names(top_btwnMaryam[10]),names(top_btwnMaryam[11]),names(top_btwnMaryam[12]),names(top_btwnMaryam[13]),names(top_btwnMaryam[14]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_btwnMaryam[15]),names(top_btwnMaryam[16]),names(top_btwnMaryam[17]),names(top_btwnMaryam[18]),names(top_btwnMaryam[19]),names(top_btwnMaryam[20]),sep=" ,")`}|
| By prestige centrality | \textit{`r paste(names(top_evcentMaryam[1]), names(top_evcentMaryam[2]),names(top_evcentMaryam[3]),names(top_evcentMaryam[4]),names(top_evcentMaryam[5]),names(top_evcentMaryam[6]),names(top_evcentMaryam[7]), sep = " ,")`} | 
|                    | \textit{`r paste(names(top_evcentMaryam[8]),names(top_evcentMaryam[9]),names(top_evcentMaryam[10]),names(top_evcentMaryam[11]),names(top_evcentMaryam[12]),names(top_evcentMaryam[13]),names(top_evcentMaryam[14]), sep = " ,")`} |
|                    | \textit{`r paste(names(top_evcentMaryam[15]),names(top_evcentMaryam[16]),names(top_evcentMaryam[17]),names(top_evcentMaryam[18]),names(top_evcentMaryam[19]),names(top_evcentMaryam[20]), sep=" ,")`}|
...
## Word collocations statistical method
...
## Lexical diversity and dispersion
...
## Viewing the network as dendrogram
...
\chapter{Text Classification Models}\label{chap:chap8}
...
For example, as we have shown in Chapter \@ref(chap:chap3), sentiment scoring is clearly model-dependent; the results vary if we vary the model used. Whether a pre-built model is a good scoring method for Quran Analytics is yet to be ascertained.
...
| Topic   | STM                                 | LDA                                   |
|---------|-------------------------------------|---------------------------------------|
| Rank 1  | (Topic 5) allah, day, muhammad      | (Topic 4) people, bring, deeds        |
| Rank 2  | (Topic 1) lord, earth, mercy        | (Topic 3) lord, found, earth          |
| Rank 3  | (Topic 6) people, cave, sea         | (Topic 6) day, muhammad, warn         |
| Rank 4  | (Topic 4) deeds, found, remained    | (Topic 2) cave, righteous, mention    |
| Rank 5  | (Topic 3) moses, righteous, life    | (Topic 1)  lord, moses, worldly       |
| Rank 6  | (Topic 2) moses, righteous, guidance | (Topic 5)  allah, remained, mercy    |
...
| Topic   | STM                     | LDA                    |        LSA              |
|---------|-------------------------|------------------------|-------------------------| 
| Rank 1  | (Topic 5) allah, day, muhammad      | (Topic 4) people, bring, deeds | (Topic 1) lord, allah, mercy |
| Rank 2  | (Topic 1) lord, earth, mercy        | (Topic 3) lord, found, earth    | (Topic 5) adorned, resting, wills |
| Rank 3  | (Topic 6) people, cave, sea         | (Topic 6) day, muhammad, warn   | (Topic 6) wills, truth, disbelieve |
| Rank 4  | (Topic 4) deeds, found, remained    | (Topic 2) cave, righteous, mention | (Topic 4) allah, cave, guide |
| Rank 5  | (Topic 3) moses, righteous, life    | (Topic 1)  lord, moses, worldly | (Topic 3) people, lord, mercy |
| Rank 6  | (Topic 2) moses, righteous, guidance | (Topic 5)  allah, remained, mercy | (Topic 2) lord, words, righteous |
...
\chapter{Knowledge Through Verse Network}\label{chap:chap9}
...
## Tafseer Ibnu Katheer as Knowledge Graphs
...
We can benchmark the results from the above table with similar tables in Chapter \@ref(chap:chap7) for Surah Al-Kahf and Surah Maryam. Note that the measures are for word co-occurrences network, which is a different framework than what we have here.
...
## Traversals in verses 2:255 and 16:90
...
## Word co-occurrences from Katheer Graph
...
<!--chapter:end:12-Ch9KnowledgeGraphV2.Rmd-->

\part{Way Forward}\label{part:part4}
...
\section*{New tools for studying Al-Quran}
...
\section*{Direction of future works}
